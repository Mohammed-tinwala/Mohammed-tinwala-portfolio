    ...
    <div className='group relative bg-white/5 border border-white/10 rounded-2xl overflow-hidden hover:border-primary/30 transition-all duration-300'>
      <div className='relative h-64 overflow-hidden'>
        ...
        <div className='absolute top-4 left-4'>
          <span className='px-3 py-1 text-xs font-medium text-white bg-black/40 backdrop-blur-sm border border-white/20 rouded-full'>
            {project.category}
          </span>
        </div>
      </div>
      ...
      <div className='p-6 space-y-4'>
        ...
        <div className='flex flex-wrap gap-2'>
          {technologies.map((tech, index) => {
            <span
              key={index}
              className='px-3 py-1 text-xs font-medium text-primary bg-primary/10 border border-primary/20 rounded-lg hover:bg-primary/20 transition-color duration-200'
            >
              {tech}
            </span>
          })}
